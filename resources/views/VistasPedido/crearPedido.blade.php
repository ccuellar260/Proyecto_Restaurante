...
@section('Contenido')
    ...
    <div class="flex items-center justify-between text-sm tracking-widest uppercase ">
        ...
        <div class="flex items-center">
            ...
            <form action="{{ Route('Pedido.storePedido') }}" method="POST">
                @csrf
                <p class="text-gray-500 dark:text-gray-300">
                    <input
                        ...
                        type="submit" value="Guardar Pedido">
                </p>

                <input type="hidden" name="empleado" value="{{ $ci }}">
                <input type="hidden" name="mesa" value="{{ $nro_mesa }}">
                ...
                <p>
                    ...
                    <div class="grid gap-1 grid-cols-2">
                        <label
                            ...
                            <input
                                class="shadow  border rounded  py-1 px-1 text-gray-700 w-16
                           leading-tight focus:outline-none' focus:shadow-outline"
                                type="number" name='cantidad[]' id="cantidad" value="0">
    ...
    <div class="grid grid-cols-1 gap-8 mt-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        ...
        @foreach ($bebidas as $b)
            ...
            <div class="flex flex-col items-center justify-center w-full max-w-lg mx-auto">
                ...
                <p>
                    <!-- usando el status solo muestra un mensaje-->
                    @if (session("status{$b->id_producto}"))
                        <!--Existe el atributo status?-->
                        <!-- mostarr lo que esta en status-->
                ...
                <input type="hidden" name="fecha" value="{{ date('Y-m-d') }}">
                <input type="hidden" name="hora" value="{{ date('H:i:s') }}">
                <input type="hidden" name="precio[]" value="{{ $b->precio }}">
                <input type="hidden" name="producto[]" value="{{ $b->id_producto }}">
    ...
    <div class="grid grid-cols-1 gap-8 mt-8 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        ...
        @foreach ($postres as $po)
            ...
            <div class="flex flex-col items-center justify-center w-full max-w-lg mx-auto">
                ...
                <p>
                    <!-- usando el status solo muestra un mensaje-->
                    @if (session("status{$po->id_producto}"))
                        <!--Existe el atributo status?-->
                        <!-- mostarr lo que esta en status-->
                ...
                <input type="hidden" name="fecha" value="{{ date('Y-m-d') }}">
                <input type="hidden" name="hora" value="{{ date('H:i:s') }}">
                <input type="hidden" name="precio[]" value="{{ $po->precio }}">
                <input type="hidden" name="producto[]" value="{{ $po->id_producto }}">
