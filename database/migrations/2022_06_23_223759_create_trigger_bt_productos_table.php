...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        FOR EACH ROW update productos set cantidad = cantidad - new.cantidad where id_producto = new.id_producto
       ');
        ...
        VALUES ('chris','insertar',new.id_producto,new.nombre,new.precio,new.cantidad)");

        DB::unprepared("
        CREATE TRIGGER `TEditProducto` AFTER UPDATE ON `productos`
        FOR EACH ROW INSERT INTO bitacora_productos(user,accion,id_producto,nombre,precio,cantidad)
        VALUES ('chris','edit',old.id_producto,old.nombre,old.precio,old.cantidad)");

        DB::unprepared("
        CREATE TRIGGER `TDeleteProducto` AFTER DELETE ON `productos`
        FOR EACH ROW INSERT INTO bitacora_productos(user,accion,id_producto,nombre,precio,cantidad)
        VALUES ('chris','delete',old.id_producto,old.nombre,old.precio,old.cantidad)");


        DB::unprepared("
        CREATE TRIGGER `TEditPedido` AFTER UPDATE ON `pedidos`
        FOR EACH ROW INSERT INTO bitacora_pedidos(user,accion,id_pedido,nro_mesa,ci_empleado)
        VALUES ('chris','edit',old.id_pedido,old.nro_mesa,old.ci_empleado)");
